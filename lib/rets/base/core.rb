  ...
  module Base
    class Core
      ...
      def search(args, &block)
        ...
        @http.request(:url => @urls[:search], :read_timeout => args[:read_timeout], :params => {:Format => (args[:format] || "COMPACT-DECODED"), :SearchType => args[:search_type], :StandardNames => (args[:standard_names] && 1 || 0), :QueryType => "DMQL2", :Query => args[:query], :Class => args[:class], :Limit => args[:limit]}) do |response|
          stream = RETS::StreamHTTP.new(response)
